...
import com.lei123.YSPocketTools.http.HTTPs.SignHttp.issign
        ...
        for (i in 0..9) {
            ...
            if (uids != "123456789" && cookies != "123456789" && uids != mainUID) {
                ...
                viewModel.signMessage = viewModel.signMessage + loadString(uids+"usernickname") + "：" +
                        home_Sign_simply(uids, cookies,0) + "\n"
                Thread.sleep(500)
...
fun home_Sign_complex(uid: String, cookie: String): String {
    var signMessage = home_Sign_simply(uid, cookie,0)
...
fun home_Sign_simply(uid: String, cookie: String, count:Int): String {
    val ifsign = issign(uid,cookie)
    Log.i("ifsign", ifsign)
    ...
    if (ifsign!="true"){
        ...
        if (result != "failed") {
            ...
            if (signMessage=="OK"){
                val risk_code = SignResult.data?.risk_code
                if (risk_code=="0"){
                    signMessage = getString(R.string.signSuccess)
                }else{
                    if (count < 10){
                        if(loadString("ifToast", getString(R.string.Yes))==getString(R.string.Yes)) "uid:${uid}签到遇到验证码、2~3秒钟之后重试第${count+1}次".toast()
                        Thread.sleep((2000..3000).random().toLong())
                        home_Sign_simply(uid,cookie,count+1)
                    }else{
                        signMessage = "重试次数过多，请稍后重试"
                        if(loadString("ifToast", getString(R.string.Yes))==getString(R.string.Yes)) signMessage.toast()
                    }
                }
            }
        }
